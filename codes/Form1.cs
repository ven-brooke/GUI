...
namespace GUI
{
    public partial class FormMain : Form
    {
        ...
        {
            // Logic to generate the next invoice number based on your business rules
            // For example, you can use a combination of prefix, suffix, and a sequential number
            // Here's a basic example that uses a prefix "INV" and a sequential number retrieved from the "Receipts" table

            OleDbConnection connection = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\\Users\\USER\\Documents\\Y2_S2\\CPE262_OOP2\\GUI\\GUIdatabase.accdb");
            connection.Open();
            OleDbCommand cmd = new OleDbCommand("SELECT MAX(InvoiceNumber) FROM Receipts", connection);
            object result = cmd.ExecuteScalar();
            ...
            string invoiceNumber = "INV" + nextInvoiceNumber.ToString().PadLeft(4, '0'); // Format the invoice number with a prefix "INV" and 4-digit sequential number
            return invoiceNumber;
        }

        // Method to get the selected items from the "Orders" table
        private string GetSelectedItems()
        {
            string selectedItems = string.Empty;
            OleDbConnection connection = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\\Users\\USER\\Documents\\Y2_S2\\CPE262_OOP2\\GUI\\GUIdatabase.accdb");
            connection.Open();
            OleDbCommand cmd = new OleDbCommand("SELECT ItemOrder FROM Orders WHERE ItemSelected = True", connection);
            OleDbDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
                selectedItems += reader["ItemOrder"].ToString() + ", "; // Concatenate the selected item names
            }
            connection.Close();

            if (!string.IsNullOrEmpty(selectedItems))
            {
                selectedItems = selectedItems.TrimEnd(',', ' '); // Remove the trailing comma and space
            }

            return selectedItems;
        }

        // Method to calculate the total price of the selected items from the "Orders" table
        private decimal CalculateTotalPrice()
        {
            decimal totalPrice = 0;
            OleDbConnection connection = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\\Users\\USER\\Documents\\Y2_S2\\CPE262_OOP2\\GUI\\GUIdatabase.accdb");
            connection.Open();
            OleDbCommand cmd = new OleDbCommand("SELECT SUM(ItemSubtotal) FROM Orders WHERE ItemSelected = True", connection);
            object result = cmd.ExecuteScalar();
            if (result != DBNull.Value)
            {
                totalPrice = Convert.ToDecimal(result); // Get the sum of the "ItemSubtotal" column as the total price
            }
            connection.Close();
